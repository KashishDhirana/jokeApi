class JokeApi {
    ...
    validateElements() {
        const elements = [this.jokeArea, this.categoriesSelection, this.typesSelection, this.submitButton, this.safeMode, this.amount]
    ...
    setupEventListeners() {
        this.submitButton?.addEventListener('click', () => this.fetchJokes())
     ...
     */
    handleAmount(e) {
        if (!(e.target instanceof HTMLButtonElement) || !(this.amount instanceof HTMLInputElement)) {
    ...
    async fetchJokes() {
        if (!(this.categoriesSelection instanceof HTMLSelectElement) || !(this.typesSelection instanceof HTMLSelectElement) || !(this.safeMode instanceof HTMLInputElement) || !(this.amount instanceof HTMLInputElement)) {
    ...
    renderJokes() {
        ...
        this.jokes.forEach(joke => {
            ...
            li.innerHTML = joke.type === 'single'
                ? `${joke.joke} <br/><span>[Category]: ${joke.category} | [NSFW]: ${!joke.safe}</span><br/><br/><hr><br/>`
                : `[Setup]: ${joke.setup} <br/>[Delivery]: ${joke.delivery} <br/><span>[Category]: ${joke.category} | [NSFW]: ${!joke.safe}</span><br/><br/><hr><br/>`
            ul.appendChild(li)
        })
